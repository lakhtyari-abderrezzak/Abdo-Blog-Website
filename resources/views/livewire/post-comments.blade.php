<div class="mt-10 comments-box border-t border-gray-100 pt-10">
    <h2 class="text-2xl font-semibold text-gray-900 mb-5">{{ __('blog_lang.discussions') }}</h2>
    <textarea
        wire:model='comment' 
        ...
        cols="30" rows="7" @guest disabled @endguest></textarea>
    @guest
        <div class="div">
            <a class="text-yellow-500 underline py-1" href="{{ route('login') }}"> {{ __('blog_lang.login_comment')}} </a>
        </div>
    @endguest
    @auth
        <button
    ...
    <div class="user-comments px-3 py-2 mt-5">
        @forelse ($this->comments as $comment)
            <div class="comment [&:not(:last-child)]:border-b border-gray-100 py-5">
                <div class="user-meta flex mb-4 text-sm items-center">
                    <x-posts.author size="md" :author="$comment->user" />
        ...
        @empty
            <div class="text-gray-500 text-center">
                <span>{{ __('blog_lang.no_comments') }}</span>
            </div>
        @endforelse
